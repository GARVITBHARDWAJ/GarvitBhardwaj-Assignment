...
def main():
    ...
    while(1):
        ...
        print("1-Create\n2-Read\n3-Delete\n4-Show all\n5-Exit\n")   #choose number to perform specific operation
        opt=input
        if(opt==1):
            c = create()
            return(c)
    ...
    t1=Thread(target=(create or read or delete))                    # THREAD added
    t1.start()
    time.sleep(1)
    ...
    f=open("garvit.txt",'w')                                         #Storing the data in given file.
    strc=str(d)                                                      #User can also provide the path for specific file.           
    f.write(strc)
    f=open("garvit.txt",'r')
    print(f.read())
    f.close()
